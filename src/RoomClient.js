...
/** Media handlers */
import { MicrophoneHandler } from './room-client/microphone-handler'
...
const DEFAULT_SIMULCAST_OPTIONS =
{
  low: 100000,
  medium: 300000,
  high: 1500000
...
export default class RoomClient {
  constructor (
    { mediaServerWSS, roomId, peerName, displayName, device, useSimulcast, produce, dispatch, getState, turnservers, args }) {
    logger.debug(
      'constructor() [roomId:"%s", peerName:"%s", displayName:"%s", device:%s]',
      roomId, peerName, displayName, device.flag)
    const protooUrl = getProtooUrl(mediaServerWSS, peerName, roomId)
  ...
  changeWebcam () {
    ...
    return Promise.resolve()
      ...
      .then(() => {
        ...
        return this._getWebcamStream(device, resolution)
      })
      .then((stream) => {
  ...
  _changeScreenForShare () {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        return this._getScreenShareStream(this._screenStreamId)
      })
      .then((stream) => {
  ...
  _setScreenShareProducer () {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        return this._getScreenShareStream()
      })
      .then((stream) => {
        this._screenShareOriginalStream = stream
   ...
   */
  async _getScreenShareStream (screenStreamId, width = 1280, height = 720) {
    return navigator.mediaDevices.getUserMedia({
      audio: false,
      video: {
        mandatory: {
   ...
   * @return {Promise<void>}
   */
  async _setWebcam (device) {
    this._dispatch(
      stateActions.setWebcamInProgress(true))
    this._webcam.device = device

    const resolution = this._webcam.resolution
    const stream = await this._getWebcamStream(device, resolution)
   ...
   * @return {Promise<MediaStream>}
   */
  _getWebcamStream (device, resolution) {
    return navigator.mediaDevices.getUserMedia({
  ...
  record (interval) {
    ...
    const videoOptions = { mimeType: 'video/webcam; codecs=vp8' }
    const audioOptions = { mimeType: 'audio/ogg; codecs=opus' }
    ...
    function uploadBlob (recorder, blob, datatype, index) {
      ...
      const url = 'http://127.0.0.1:5000/data-' + datatype
      axios.post(url, data)
        .then((res) => {
